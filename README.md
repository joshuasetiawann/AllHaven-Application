...
> Numbering: `0.1 → 0.9`, then rolls over to `1.0 → 1.9` (so the folders sort and
> read naturally). A new update adds the next folder (`AllHaven 2.0`, …).
...
| **AllHaven 1.9** | **AllHaven** | **Current.** **Public-launch auth** — HttpOnly **cookie sessions** (hashed server-side, rotation via refresh, revocation on logout), **CSRF** double-submit, `/auth/*` **rate limiting**, and a production **SECRET_KEY guard**. No auth token in localStorage anymore. Internally semantic `v0.7.0`. |

## How the two branches relate

- **`main`** — only the **latest** version (`AllHaven 1.9`), flat at the repo root, ready to run/deploy.
