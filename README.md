...
![Latest](https://img.shields.io/badge/latest-AllHaven%202.6%20·%20v0.14.0-18E0D6?style=flat-square)
&nbsp;![Releases](https://img.shields.io/badge/releases-26-2563EB?style=flat-square)
...
| **⭐ AllHaven 2.6** | 2026-06-11 | **AllHaven** | `v0.14.0` | **Current.** **Terminal-only install** — install & start run **entirely in the terminal** again (live Docker/`pip`/`npm` progress); the browser wizard is now **opt-in** (`HAVEN_SETUP_WEB=1`), and the Docker check is faster (4s). The desktop shortcut starts services & opens Haven with no terminal. |
...
its own; the latest (`AllHaven 2.6`) keeps its own `CHANGELOG.md`, `VERSION`, and
