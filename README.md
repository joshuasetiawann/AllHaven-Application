...
> Numbering: `0.1 → 0.9`, then rolls over to `1.0 → 1.4` (so the folders sort and
> read naturally). A new big update adds the next folder (`AllHaven 1.5`, …).
...
| **AllHaven 1.4** | **AllHaven** | **Current.** Rebrand to AllHaven + responsive UI, **multi-agent Debate** mode, and the **Reasoning Quality Layer** (Analyst → Critic → Synthesizer with grounded, verified reasoning). Internally tracked as semantic `v0.3.0`. |

## How the two branches relate

- **`main`** — only the **latest** version (`AllHaven 1.4`), flat at the repo root, ready to run/deploy.
...
latest one (`AllHaven 1.4`) keeps its own detailed `CHANGELOG.md`, `VERSION`, and
