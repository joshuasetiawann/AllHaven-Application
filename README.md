...
| **AllHaven 2.2** | **AllHaven** | **Reliable one-click startup + responsive menu** — the launcher/wizard now start services faithfully like `allhaven.sh` (bind `0.0.0.0`, wait for PostgreSQL, run migrations, health-gate the backend, install deps on first run, surface logs), fixing *"works when run manually but not from the app"*. Plus a collapsible, responsive navigation menu (desktop rail, tablet icon rail, mobile drawer, user chip). Internally semantic `v0.10.0`. |
| **AllHaven 2.3** | **AllHaven** | **Current.** **Terminal installer + config sync** — the `START_HAVEN_*` launchers now install & start from the **terminal by default** with live progress (Docker pull, `pip`, `npm`); `backend/.env` now mirrors the root `.env`; and the Docker check is faster (no more frozen "checking Docker"). The browser wizard remains available via `HAVEN_SETUP_WEB=1`. Internally semantic `v0.11.0`. |

## How the two branches relate

- **`main`** — only the **latest** version (`AllHaven 2.3`), flat at the repo root, ready to run/deploy.
...
latest one (`AllHaven 2.3`) keeps its own detailed `CHANGELOG.md`, `VERSION`, and
