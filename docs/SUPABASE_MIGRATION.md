...
|---|---|---|---|
| **0016** | `backend/alembic/versions/0016_provision_me.py` (= `docs/sql/provision_me.sql`) | `public.provision_me(p_full_name text)` — `SECURITY DEFINER` RPC. Creates profile + workspace + owner membership for a brand-new signed-up user. Idempotent; adopts an existing same-email profile (desktop-first accounts) instead of duplicating. | `drop function` |
...
2. Paste the entire contents of **`docs/sql/provision_me.sql`**.
3. **Run.**
...
- `docs/sql/provision_me.sql` already ends with:
...
3. **Wrong signature** → the function is `provision_me(p_full_name text default null)`.
   ...
   re-running `docs/sql/provision_me.sql` cleanly recreates it.
...
- SQL hotfix (0016): `docs/sql/provision_me.sql`
