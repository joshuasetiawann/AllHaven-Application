...
def decode_supabase_token(token: str) -> dict:
    """Decode and verify a Supabase GoTrue session JWT (HS256).

    Supabase signs user ``access_token``s with the project's JWT secret. Verifying
    them here lets a Supabase-logged-in mobile client authenticate to the desktop
    Backend Bridge; identity is then mapped to a local Profile via
    ``supabase_user_id`` (see :mod:`app.api.dependencies`). Returns the claims or
    raises ``ValueError``.

    Only legacy HS256 (shared-secret) Supabase projects are supported; projects
    using the newer asymmetric signing keys would need JWKS verification instead.
    """
    secret = settings.SUPABASE_JWT_SECRET
    if not secret:
        raise ValueError("Supabase token auth is not configured")
    ...
    if header.get("alg") != "HS256":
        # Defends against alg-confusion ("none"/RS256) and unsupported projects.
    ...
    if int(payload.get("exp", 0)) < int(time.time()):
        raise ValueError("Token expired")
    # Supabase user tokens carry aud="authenticated" (string or list form).
    aud = payload.get("aud")
    if aud is not None:
