"""Repeat the final Supabase security reconciliation at a new immutable head.

Revision ID: 0026_security_closure
Revises: 0025_tenant_fk_reconcile
Create Date: 2026-08-13

Revision 0023 introduced the first reconciliation, but its final policy cleanup
and profile-claim hardening were strengthened during remediation. Databases that
already recorded 0023 must receive those changes through a new revision rather
than relying on edits to migration history. This revision is deliberately
idempotent and only runs when the actual bind contains Supabase ``auth.users``.
"""
...
revision: str = "0026_security_closure"
down_revision: Union[str, None] = "0025_tenant_fk_reconcile"
...
_WORKSPACE_TABLES = [
    ...
    "chat_groups", "chat_sessions", "chat_messages", "ai_tool_proposals",
    "ai_tool_calls", "ai_multi_agent_runs", "ai_agent_responses", "ai_memories",
    "ai_memory_suggestions", "ai_conversation_summaries", "ai_knowledge_documents",
    "ai_knowledge_chunks", "sync_state",
]
_DENY_TABLES = [
    "local_users",
    "user_sessions",
...
ALTER FUNCTION public.app_user_id() OWNER TO CURRENT_USER;
ALTER FUNCTION public.is_member(uuid) OWNER TO CURRENT_USER;
...
ALTER FUNCTION public.provision_me(text) OWNER TO CURRENT_USER;
...
def _reset_policies(table: str, policies: list[str]) -> None:
...
def upgrade() -> None:
    ...
    _reset_policies("workspaces", [
        'CREATE POLICY p_owner ON public."workspaces" FOR ALL TO authenticated '
        "USING (owner_id = public.app_user_id()) "
        "WITH CHECK (owner_id = public.app_user_id());",
    ])
    _reset_policies("workspace_members", [
        'CREATE POLICY p_member ON public."workspace_members" FOR SELECT TO authenticated '
        "USING (user_id = public.app_user_id());",
    ])
    _reset_policies("audit_logs", [
...
def downgrade() -> None:
    # Reconciliation cannot reconstruct arbitrary insecure policy state and the
    # function/policies predate this revision, so retaining them is the only safe
    # downgrade behavior.
    pass
