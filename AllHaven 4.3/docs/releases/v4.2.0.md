...
- The entire UI is restyled on the **Aurora Glass** design system: shared tokens and primitives in `globals.css`, glass surfaces, and aurora accents across every page and component. Visual-only — routes, APIs, and data are untouched. A design handoff bundle lives in `docs/design/aurora/`.
